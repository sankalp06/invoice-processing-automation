...
def _enhance_page(pil_image: Image.Image) -> Image.Image:
    """Apply CLAHE contrast + gamma correction to a single page image."""
...
def _enhance_pdf(input_pdf: Path, output_pdf: Path, dpi: int = 300) -> None:
    """Convert PDF pages to enhanced grayscale images and save as a new PDF."""
    pages = _pdf_to_images(input_pdf, dpi=dpi)
    enhanced = [_enhance_page(p) for p in pages]
    enhanced[0].save(output_pdf, save_all=True, append_images=enhanced[1:])
    logger.debug("Enhanced PDF saved: %s", output_pdf.name)
...
def _run_ocr(input_pdf: Path, output_pdf: Path) -> None:
    """Run ocrmypdf on *input_pdf* → *output_pdf*."""
...
def process_files(
    ...
    tgt_cont = target_container or settings.target_container_ocr
    fld_cont = failed_container or settings.failed_container_ocr
